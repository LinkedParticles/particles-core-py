...
truth axis (confidence): it measures how *useful* a belief has proven
...
lower yet governs behaviour every session (§Context).

This module is **pure** (no I/O). It owns two quantities, mirroring the
facts-vs-judgment split given to decay:

- :func:`reinforcement_score` — from the *utility evidence* (per-belief,
  store-local: the ages of the utility events), a
  ...
  when it stops being reinforced" mechanic. Two channels feed it: the transcript miner (``w = 1``) and the explicit operator gesture
  (``w = utility.explicit_weight``), which carries more because it fires once
  where the miner fires per session.
- :func:`utility_rank_bonus` — from the *utility policy* (per-observer,
  portable: the lens ``utility_rules`` ``rank_lift``), an
  **additive** term on the projection *ordering* score. **Promotion-only** by
  construction (``λ ≥ 0``, ``R ≥ 0`` ⇒ bonus ``≥ 0``): lack of use never demotes
  a correct-but-new belief; it only withholds the bonus (preserved).

This supersedes the earlier design: utility was a bounded, saturating
...
count magnitude survives monotonically.

The bonus is added to effective confidence **only on the projection / digest
ranking path** — never the semantic-search retrieval rank, and
never the stored ``confidence.value`` or the read-time ``effective_confidence``
. The resulting ``rank_score`` is an *ordering* score, **not** a
probability: it is not confined to ``[0, 1]``. The resolution of the policy
parameters and the store read of the event ages live in the Engine
...
def reinforcement_score(
    ...
    a year ago. Returns ``0.0`` for no events (cold-start neutral).
    ...
    **Why the weight is per-event.** The two channels that produce
    ...
    not mean equal influence. The miner emits one event per (belief,
    ...
    the half-life already works that way (derives ``R`` from event ages
    at read time), which keeps the channel weight retunable — and sweepable by
    the calibration harness — without rewriting history.
    ...
    Args:
        ...
        half_life_uses_days: Days for a single event's weight to halve — the
            reinforcement half-life (a lens judgment). Must be > 0.
...
def utility_rank_bonus(reinforcement: float, lambda_: float) -> float:
    """Additive projection rank-lift: ``λ · ln(1 + R)``.
    ...
    magnitude the exponential discarded. Growth is logarithmic, so
    influence is bounded without a hard ceiling: doubling reinforcement adds
    only ``λ·ln 2 ≈ 0.69λ``, so no single popular belief can run away with the
    head (the guarantee bought with ``cap``).

    **Promotion-only** (preserved): with ``λ ≥ 0`` and ``R ≥ 0``
...
# ---------------------------------------------------------------------------
#
# declined auto-fitting ``λ`` — measured on a real store, every
# candidate closed form either overshot the admissible band by 3–8×, returned
# zero (the cap flattens the head's confidence spread to exactly
...
class SweepRow:
    ...
    Attributes:
        ...
        effective_confidence: The truth-axis score with the utility bonus
            **off** — the base the lift is added to.
        ...
        owner_relevant: ``A(p)`` — whether the belief is about the viewer
            . Unused by the ``λ`` sweep; consumed by
...
class RankLiftSweep:
    ...
    Attributes:
        ...
        intersection: The band of ``λ`` values admissible for **every** surface.
            Empty when the surfaces cannot be satisfied at once — the finding, which is a real property of the store rather than a
...
def sweep_rank_lift(
    ...
    *,
    ...
    "two-map recipe") as :class:`SweepRow` values, and this ranks them by
    ``effective_confidence + λ·ln(1 + R)`` with the digest's ``(-score, id)``
