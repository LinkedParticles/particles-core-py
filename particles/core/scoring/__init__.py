...
merge), :mod:`~particles.core.scoring.decay` (content-age recency
decay), :mod:`~particles.core.scoring.utility` (usefulness
reinforcement + additive rank-lift), and
:mod:`~particles.core.scoring.relevance` (owner-aboutness rank-lift).
The last three are the three read-time axes — **truth**, **use**, **aboutness**
— composed as separate addends on one ordering score. These are Client-layer
math: the Engine composition that reads the store and calls them
lives in ``operations/query/effective_confidence.py``. See this package's
``AGENTS.md``.
