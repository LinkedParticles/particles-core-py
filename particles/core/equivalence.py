...
confidence within a store, or a computed similarity across stores.
It is the claim-identity sibling of ``compute_effective_confidence``:
an observer-neutral value read through a per-observer trust term at query time,
never stored.
...
its exact formula and the per-observer trust-policy surface are deferred, along with cross-lingual detection and the ``language`` field.
"""
...
def effective_equivalence(link_confidence: float, *, observer_trust: float | None = None) -> float:
    """Observer-relative claim-equivalence strength in ``[0, 1]``.
